...
import { getAllCategories, getAllSubcategories, getAllGroups } from '../../utils/supabaseApi';
import './style.css';
...
const CategoriesMenu = () => {
  ...
  useEffect(() => {
    ...
    const checkAnnouncementBar = () => {
      ...
      const hasAnnouncement = announcementBar && announcementBar.offsetHeight > 0;
  ...
  const handleSearchClick = () => {
    ...
    closeMenu();
    setTimeout(() => {
      const input = document.querySelector('.desktop-search-overlay input[type="search"], .desktop-search-overlay input[type="text"]');
  ...
  if (loading) {
    ...
    <>
      ...
      {menuPortalTarget && createPortal(
        <>
          {/* Overlay */}
          {isMenuOpen && (
            <div className={`categories-menu-overlay ${hasAnnouncementBar ? 'with-announcement' : ''}`} onClick={closeMenu} />
          )}

          {/* Slide-out Menu */}
          <div className={`categories-menu-panel ${isMenuOpen ? 'open' : ''} ${hasAnnouncementBar ? 'with-announcement' : ''}`}>
            <div className="categories-menu-content">
              ...
              <div className="categories-list">
                ...
                <div className="menu-section">
                  <div style={{
                    ...
                    <form onSubmit={handleSearchSubmit} style={{
                      ...
                      <MdSearch size={18} style={{ color: '#666', flexShrink: 0, marginRight: '8px' }} />
                      <input
                        type="search"
                ...
                {/* Shop by Category */}
                <div className="menu-section">
                  <div className="menu-item category-trigger" onClick={() => setIsShopByCategoryExpanded(!isShopByCategoryExpanded)}>
                    <span className="nav-item-name">Shop by Category</span>
                    <button className="plus-icon-btn" aria-label={isShopByCategoryExpanded ? 'Collapse' : 'Expand'}>
                      <span className="plus-icon">{isShopByCategoryExpanded ? '−' : '+'}</span>
                    </button>
                  </div>
                            ...
                            {isExpanded && subcats.length > 0 && (
                              <div className="subcategories-group">
                                {subcats.map((subcategory) => {
                                  const isSubcatExpanded = expandedSubcategories.includes(subcategory.id);
                                  return (
                                    <div key={subcategory.id} className="subcategory-group">
                                      <div className="menu-item subcategory-item">
                                        ...
                                        <button
                                          ...
                                          aria-label={isSubcatExpanded ? 'Collapse subcategory' : 'Expand subcategory'}
                                        >
                                          <span className="plus-icon">{isSubcatExpanded ? '−' : '+'}</span>
                                        </button>
                                      </div>
                                      {/* Groups under subcategory - only show if subcategory is expanded */}
                                      {isSubcatExpanded && (
                                        <div className="groups-list">
                                          {getGroupsForSubcategory(subcategory.id).map(group => (
                                            <Link
                                              key={group.id}
      ...
      {searchOverlayOpen && createPortal(
        <div className="desktop-search-overlay open" style={{position: 'fixed', zIndex: 2000, top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255,255,255,0.98)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: 80}}>
          <button onClick={() => setSearchOverlayOpen(false)} style={{position: 'absolute', top: 20, right: 20, background: 'none', border: 'none', fontSize: 28, color: '#333', zIndex: 1}} aria-label="Close search">×</button>
          <form onSubmit={handleSearchSubmit} style={{width: '90%', maxWidth: 500, marginTop: 40, display: 'flex', gap: 12}}>
            <input
              type="search"
              placeholder="Search products..."
              style={{width: '100%', padding: '16px 20px', fontSize: 18, borderRadius: 8, border: '2px solid #ddd', outline: 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.1)'}}
              autoFocus
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleSearchSubmit(e); }}
            />
            <button type="submit" style={{padding: '0 24px', fontSize: 16, borderRadius: 8, border: 'none', background: '#007bff', color: '#fff', fontWeight: 500, cursor: 'pointer', height: 56, minWidth: 100}}>Search</button>
          </form>
        </div>,
        document.body
      )}
