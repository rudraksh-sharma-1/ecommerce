...
import { getAllCategories, getAllSubcategories, getAllGroups, getActiveCategories } from '../../utils/supabaseApi';
import { useAuth } from '../../contexts/AuthContext';
import CategoriesMenu from '../CategoriesMenu';
import { GiHamburgerMenu } from 'react-icons/gi';
import { MdClose } from 'react-icons/md';
import { FaHome } from 'react-icons/fa';
import { MdCategory, MdOutlineShoppingCart, MdMenu, MdSearch } from 'react-icons/md';
import './style.css';
...
const CategoriesBar = () => {
  ...
  const [dropdownRender, setDropdownRender] = useState({ categoryId: null, position: { top: 0, left: 0 } });
  const [groupsDropdownRender, setGroupsDropdownRender] = useState({ subcategoryId: null, position: { top: 0, left: 0 } });
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      if (hoveredCategoryRef.current) {
        const categoryElement = categoryRefs.current[hoveredCategoryRef.current];
        if (categoryElement) {
  ...
  const fetchCategoriesSubcategoriesAndGroups = async () => {
    ...
    try {
      ...
      if (categoriesResult.success && categoriesResult.categories) {
        ...
        const activeCategories = categoriesResult.categories.filter(cat => cat.active);
        setAllCategories(activeCategories);
      }

      if (subcategoriesResult.success && subcategoriesResult.subcategories) {
        setSubcategories(subcategoriesResult.subcategories.filter(sub => sub.active));
      }

      if (groupsResult.success && groupsResult.groups) {
        const activeGroups = groupsResult.groups.filter(group => group.active);
        setGroups(activeGroups);
      }
    } catch (error) {
      console.error('Error fetching categories, subcategories, and groups:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getGroupsForSubcategory = (subcategoryId) => {
    const groupsForSubcategory = groups.filter(group => group.subcategory_id === subcategoryId);
  ...
  const calculateDropdownPosition = (categoryElement) => {
    ...
    if (windowWidth > 768) {
      ...
      if (left < 8) left = 8;
      if (left + dropdownWidth > windowWidth - 8) left = windowWidth - dropdownWidth - 8;
    }
    ...
    if (top + dropdownHeight > windowHeight - 8) {
      ...
      if (top < categoriesBarRect.bottom + 4) top = categoriesBarRect.bottom + 4;
    }
  ...
  const handleCategoryMouseEnter = (categoryId, event) => {
    ...
    setGroupsDropdownRender({ subcategoryId: null, position: { top: 0, left: 0 } });
  };

  const handleCategoryMouseLeave = () => {
    isDropdownHoveredRef.current = false;
    setTimeout(() => {
      if (!isDropdownHoveredRef.current && !isGroupsDropdownHoveredRef.current) {
  ...
  useEffect(() => {
    ...
    const handleEscapeKey = (event) => {
      if (event.key === 'Escape' && (hoveredCategoryRef.current || hoveredSubcategoryRef.current)) {
        setDropdownRender({ categoryId: null, position: { top: 0, left: 0 } });
        setGroupsDropdownRender({ subcategoryId: null, position: { top: 0, left: 0 } });
      }
    };
  ...
  const handleBottomMenu = (type) => {
    ...
    if (type === 'category') {
      setMobileMenuOpen(true);
      setOpenSubmenu((prev) => prev.includes('shop-by-category') ? prev : [...prev, 'shop-by-category']);
    }
  ...
  return (
    <>
      <div className={`categories-bar ${hasAnnouncementBar ? 'with-announcement-bar' : ''}`}>
              ...
              {loading ? (
                ...
                categories.map((category) => {
                  ...
                  return (
                    <div
                      key={category.id}
                      className={`category-item-container${isSelected ? ' selected' : ''}`}
                    ...
                    >
                      ...
                      >
                        <div className="category-item-new">
                          <div className="category-name-container">
                            ...
                            {hasSubcategories && (
                              <div className="dropdown-indicator-container">
                      ...
                      {hasSubcategories && dropdownRender.categoryId === category.id && (
                        <div 
                          ...
                          style={{
                            ...
                            minWidth: dropdownRender.position.width || '340px',
                            maxWidth: '700px',
                            transform: 'none',
                        ...
                        >
                          <div className="subcategories-dropdown-content" style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
                            <div className="subcategory-column" style={{ flex: 1, minWidth: '0', maxWidth: '50%', paddingRight: '16px' }}>
                              {categorySubcategories.slice(0, Math.ceil(categorySubcategories.length / 2)).map((subcategory) => {
                                const subcategoryGroups = getGroupsForSubcategory(subcategory.id);
                                const hasGroups = subcategoryGroups.length > 0;
                                return (
                                  <div
                                    key={subcategory.id}
                                    ...
                                    onMouseEnter={(e) => {
                                      if (hasGroups) {
                                        handleSubcategoryMouseEnter(subcategory.id, e);
                                      }
                                    }}
                                    onMouseLeave={() => {
                                      if (hasGroups) {
                                  ...
                                  >
                                    <Link
                                      to={`/productListing?subcategory=${encodeURIComponent(subcategory.name)}&category=${encodeURIComponent(category.name)}`}
                            ...
                            <div className="subcategory-column" style={{ flex: 1, minWidth: '0', maxWidth: '50%', paddingLeft: '16px' }}>
                              {categorySubcategories.slice(Math.ceil(categorySubcategories.length / 2)).map((subcategory) => {
                                const subcategoryGroups = getGroupsForSubcategory(subcategory.id);
                                const hasGroups = subcategoryGroups.length > 0;
                                return (
                                  <div
      ...
      {groupsDropdownRender.subcategoryId && (
        ...
        >
          <div className="groups-dropdown-content">
            {(() => {
              const subcategory = subcategories.find(sub => sub.id === groupsDropdownRender.subcategoryId);
              const subcategoryGroups = getGroupsForSubcategory(groupsDropdownRender.subcategoryId);
              const category = categories.find(cat => cat.id === subcategory?.category_id);
              
              return subcategoryGroups.map((group) => (
                <Link
                  key={group.id}
                  to={`/productListing?group=${encodeURIComponent(group.name)}&subcategory=${encodeURIComponent(subcategory?.name || '')}&category=${encodeURIComponent(category?.name || '')}`}
      ...
      <div className={`mobile-menu-overlay ${mobileMenuOpen ? 'open' : ''}`} onClick={closeMobileMenu}>
        <div className="mobile-menu-container" onClick={(e) => {
          e.stopPropagation();
          ...
          <div className="mobile-menu-content">
            <ul className="mobile-nav-list">
              <li>
                <div style={{
                  ...
                  <form onSubmit={handleSearchSubmit} style={{
                    ...
                    <MdSearch size={18} style={{ color: '#666', flexShrink: 0 }} />
                    <input
                      type="search"
              ...
              <li className="has-submenu">
                ...
                <ul className={`mobile-submenu ${openSubmenu.includes('shop-by-category') ? 'open' : ''}`}>
                      ...
                      <li key={category.id} className="category-menu-item">
                        {categorySubcategories.length > 0 ? (
                          <div className="category-item-wrapper">
                            <div
                              className={`category-item-content ${openSubmenu.includes(category.id) ? 'expanded' : ''}`}
                              onClick={e => {
                                // Only expand/collapse if NOT clicking the text link
                                const link = e.currentTarget.querySelector('.category-text-link');
                        ...
                        {categorySubcategories.length > 0 && (
                          <ul className={`mobile-submenu ${openSubmenu.includes(category.id) ? 'open' : ''}`}>
                            {categorySubcategories.map(subcategory => {
                              const subcategoryGroups = getGroupsForSubcategory(subcategory.id);
                              return (
                                <li key={subcategory.id} className="category-menu-item">
                                  {subcategoryGroups.length > 0 ? (
                                    <div className="category-item-wrapper">
                                      <div
                                        className={`category-item-content ${openSubmenu.includes(`${category.id}-${subcategory.id}`) ? 'expanded' : ''}`}
                                        onClick={e => {
                                          const link = e.currentTarget.querySelector('.category-text-link');
                                          if (link && (e.target === link || link.contains(e.target))) {
                                            return;
                                          ...
                                          e.preventDefault();
                                          e.stopPropagation();
                                          toggleSubmenu(`${category.id}-${subcategory.id}`);
                                        }}
                                        style={{ cursor: 'pointer' }}
                                      >
                                        <Link
                                  ...
                                  {subcategoryGroups.length > 0 && (
                                    <ul className={`mobile-submenu ${openSubmenu.includes(`${category.id}-${subcategory.id}`) ? 'open' : ''}`}>
                                      <li>
                                        <Link 
                                          to={`/productListing?subcategory=${encodeURIComponent(subcategory.name)}&category=${encodeURIComponent(category.name)}`} 
                                          onClick={closeMobileMenu}
            ...
            <div className="mobile-menu-footer" style={{marginTop: 'auto', padding: '16px 16px 80px 16px', borderTop: '1px solid #eee', textAlign: 'center', fontSize: '15px'}}>
              {currentUser ? (
                <>
                  ...
                  <button
                    ...
                    style={{marginLeft: '16px', color: '#d32f2f', background: 'none', border: 'none', fontWeight: 500, cursor: 'pointer', fontSize: '15px'}}>
                    Logout
                  </button>
                </>
              ) : (
                <>
                  <Link to="/login" onClick={closeMobileMenu} style={{marginRight: '8px', color: '#007bff', textDecoration: 'none'}}>Login</Link>
                  <span style={{color: '#888'}}>|</span>
                  <Link to="/signup" onClick={closeMobileMenu} style={{marginLeft: '8px', color: '#007bff', textDecoration: 'none'}}>Register</Link>
                </>
              )}
            </div>
          </div>
      ...
      <div className="mobile-bottom-nav-bar" style={{position: 'fixed', left: 0, right: 0, bottom: 0, height: '54px', background: '#fff', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 1200}}>
        <button className="mobile-bottom-nav-btn" aria-label="Home" onClick={() => handleBottomMenu('home')} style={{background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, padding: 0}}>
          <FaHome size={22} />
          <span style={{fontSize: '11px'}}>Home</span>
        </button>
        <button className="mobile-bottom-nav-btn" aria-label="Category" onClick={() => handleBottomMenu('category')} style={{background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, padding: 0}}>
          <MdCategory size={22} />
          <span style={{fontSize: '11px'}}>Category</span>
        </button>
        <button className="mobile-bottom-nav-btn" aria-label="Cart" onClick={() => handleBottomMenu('cart')} style={{background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, padding: 0}}>
          <MdOutlineShoppingCart size={22} />
          <span style={{fontSize: '11px'}}>Cart</span>
        </button>
        <button className="mobile-bottom-nav-btn" aria-label="Menu" onClick={() => handleBottomMenu('menu')} style={{background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, padding: 0}}>
          <MdMenu size={22} />
      ...
      {searchOverlayOpen && (
        <div className="mobile-search-overlay open" style={{position: 'fixed', zIndex: 2000, top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255,255,255,0.98)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: 40}}>
          <button onClick={() => setSearchOverlayOpen(false)} style={{position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', fontSize: 28, color: '#333', zIndex: 1}} aria-label="Close search">×</button>
          <form onSubmit={handleSearchSubmit} style={{width: '90%', maxWidth: 400, marginTop: 32, display: 'flex', gap: 8}}>
            <input
              type="search"
              placeholder="Search products..."
              style={{width: '100%', padding: '12px 16px', fontSize: 18, borderRadius: 8, border: '1px solid #ccc', outline: 'none'}}
              autoFocus
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleSearchSubmit(e); }}
            />
            <button type="submit" style={{padding: '0 18px', fontSize: 16, borderRadius: 8, border: 'none', background: '#007bff', color: '#fff', fontWeight: 500, cursor: 'pointer', height: 48}}>Search</button>
          </form>
        </div>
      )}
    </>
