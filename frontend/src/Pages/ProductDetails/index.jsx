...
const ProductDetails = () => {
  ...
  const handleAddToCart = async () => {
    ...
    setCartLoading(true);
    try {
      const { success, error } = await addToCart(currentUser.id, productData.id, quantity);
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      const { success, error } = await addToWishlist(currentUser.id, productData.id);
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      setProductData(product ? {
        ...
        images: product.images ? product.images : [product.image ?? 'https://placehold.co/300x300?text=Product'],
      ...
      if (product && product.category) {
        const { success: relSuccess, products: relProducts } = await getProductsByCategory(product.category);
        if (relSuccess && relProducts) {
          setRelatedProducts(relProducts.filter(p => String(p.id) !== String(product.id)).slice(0, 4));
        } else {
          setRelatedProducts([]);
        }
      } else {
  ...
  if (!productData) {
    return <div className="container px-4 py-8 text-center">Product not found.</div>;
  }
        ...
        <div className="container px-4">
          <Breadcrumbs aria-label="breadcrumb" className="text-sm mb-4">
            ...
            <Link
              ...
              sx={{ display: "flex", alignItems: "center" }}
              color="inherit"
              href={`/productListing?category=${encodeURIComponent(productData.category || '')}`}
              className="text-gray-600 hover:text-blue-600 transition-colors"
            >
              <span className="text-xs sm:text-sm">{productData.category || 'Category'}</span>
            </Link>
            <Typography
              sx={{ display: "flex", alignItems: "center" }}
              color="text.primary"
          ...
          <div className="flex flex-col md:flex-row gap-6 md:gap-10">
            ...
            <div className="w-full md:w-[55%] lg:w-[60%]">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-6">
                ...
                >
                  {cartAdded ? 'Added!' : cartLoading ? 'Adding...' : 'Add to Cart'}
                </Button>
        ...
        <div className="container px-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {relatedProducts.length === 0 ? (
              <div className="col-span-4 text-gray-500 text-center py-8">No related products found.</div>
            ) : (
